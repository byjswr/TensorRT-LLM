...
def load_from_hf_qwen(tensorrt_llm_qwen: tensorrt_llm.models.QWenForCausalLM,
    ...
    # add
    layers_range, layers_pipeline_stage = get_transformer_layers(mapping, tensorrt_llm_qwen.num_layers)
    ...
    for k, v in tqdm(model_params.items(),
        ...
        if 'transformer.wte.weight' in k:
            ...
            if layer_idx is None or int(layer_idx) not in layers_range:
                continue
            idx = int(layer_idx) - layers_pipeline_stage
...
def get_transformer_layers(mapping, num_layers):
    ...
    num_add = num_layers % mapping.pp_size
    # 计算之前每个GPU的层数
    offset = 0
    if mapping.pp_rank < num_add:
        
        offset = 1
        offset_pre = mapping.pp_rank
    else:
        offset_pre = num_add

    layers_range = list(
        range(mapping.pp_rank * layers_per_pipeline_stage + offset_pre,
                (mapping.pp_rank + 1) * layers_per_pipeline_stage + offset_pre + offset, 1))
    return layers_range, mapping.pp_rank * layers_per_pipeline_stage + offset_pre
...
def load_from_gptq_qwen(
    ...
    elif os.path.isdir(quant_ckpt_path):
        model_params = {}
        for file in os.listdir(quant_ckpt_path):
            temp_quant_ckpt_path = os.path.join(quant_ckpt_path, file)
            if temp_quant_ckpt_path.endswith(".safetensors"):
                groupwise_qweight_safetensors = safe_open(temp_quant_ckpt_path,
                                                        framework="pt",
                                                        device="cpu")
                temp_model_params = {
                    key: groupwise_qweight_safetensors.get_tensor(key)
                ...
                for temp_key in temp_model_params.keys():
                    if temp_key in model_params:
                        print(temp_quant_ckpt_path)
                        print(temp_key)
                model_params.update(temp_model_params)        
    ...
    for layer in tqdm(layers_range,
        ...
        idx = layer - layers_pipeline_stage
