...
def read_config(config_path: Path, extra_mpirun_process = 0) -> Tuple[ModelConfig, dict]:
    """
    ...
    with open(config_path, 'r') as f:
        ...
        return _builder_to_model_config(config, 
                                        extra_mpirun_process = extra_mpirun_process)


def _builder_to_model_config(config: dict,
                             extra_mpirun_process: int = 0) -> Tuple[ModelConfig, dict]:
    ...
    assert world_size == mpi_world_size() - extra_mpirun_process, \
        f'Engine world size ({tp_size} * {pp_size}) != Runtime world size ({mpi_world_size()} - {extra_mpirun_process})'
...
class ModelRunner(ModelRunnerMixin):
    ...
    def from_dir(cls,
                 ...
                 extra_mpirun_process: int = 0) -> 'ModelRunner':
        """
