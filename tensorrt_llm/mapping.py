...
class Mapping(object):
    ...
    def pp_layers(self, num_layers):
        ...
        num_add = num_layers % self.pp_size
        # 计算之前每个GPU的层数
        offset = 0
        if self.pp_rank < num_add:
            
            offset = 1
            offset_pre = self.pp_rank
        else:
            offset_pre = num_add

        layers_range = list(
            range(self.pp_rank * layers_per_pipeline_stage + offset_pre,
                    (self.pp_rank + 1) * layers_per_pipeline_stage + offset_pre + offset, 1))
        return layers_range
